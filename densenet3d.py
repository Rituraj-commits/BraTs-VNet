...
# TODO test model
class SkipDenseNet3D(nn.Sequential):
    ...
    def __init__(self, in_channels=2, classes=4, growth_rate=16, block_config=(4, 4, 4, 4), num_init_features=32, drop_rate=0.1,
        ...
        for i, num_layers in enumerate(block_config):
            ...
            up_block = nn.ConvTranspose3d(num_features, classes+1, kernel_size=2 ** (i + 1) + 2,
                                          stride=2 ** (i + 1),
                                          padding=1, groups=classes+1, bias=False)
        ...
        # ----------------------- classifier -----------------------
        self.bn_class = nn.BatchNorm3d((classes+1) * 4 + num_init_features)
        self.conv_class = nn.Conv3d((classes+1) * 4 + num_init_features, classes, kernel_size=1, padding=0)
